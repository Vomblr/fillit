...
PROGRAM_PATH="~/Desktop/fillit/ "
LIBFT_PATH="~/Desktop/fillit/libft/"
...
for i in ${!test[@]}
do
	output=$(./fillit ${BASE_DIR}examples/${test_name[$i]}.fillit)
	runtime=$((time (./fillit ${BASE_DIR}examples/${test_name[$i]}.fillit) 2>&1) | grep ^real)
